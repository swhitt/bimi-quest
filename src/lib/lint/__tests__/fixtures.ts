/**
 * Test fixtures for lint rule tests.
 * BIMI_VMC_PEM: A real DigiCert VMC for CNN (Cable News Network, Inc.)
 * PERFECT_BIMI_VMC_PEM: Self-signed cert that passes all lint rules (generated by scripts/gen-test-cert.ts)
 * NON_BIMI_PEM: A self-signed test cert with no BIMI extensions.
 */

// DigiCert VMC for cnn.com — has BIMI EKU, logotype extension, mark type, etc.
export const BIMI_VMC_PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`;

// Self-signed VMC that passes all lint rules — generated by scripts/gen-test-cert.ts
// Validity: 824 days (max 825 per MCR §6.3.2). Regenerate when expired.
// Subject: C=US/ST=California/L=San Francisco/O=Test Brand Inc./CN=Test Brand Inc.
// Mark Type: Registered Mark, Trademark Office: USPTO, Country: US, Reg ID: 97123456
// Contains: BIMI EKU, logotype w/ gzip SVG Tiny PS, SHA-256 hash, SCT list, DigiCert policy OID
export const PERFECT_BIMI_VMC_PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;

// Self-signed test cert — no BIMI extensions, CA=true, RSA 512-bit
export const NON_BIMI_PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;
