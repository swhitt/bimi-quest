...
const svg = `<svg width="${WIDTH}" height="${HEIGHT}" xmlns="http://www.w3.org/2000/svg">
  ...
  <g transform="translate(${WIDTH / 2 - 40}, 140)">
    ...
    <path d="M28 45 L37 54 L55 36" fill="none" stroke="#A78BFA" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
  </g>

  <!-- Title -->
  <text x="${WIDTH / 2}" y="290" text-anchor="middle"
        font-family="sans-serif" font-weight="bold" font-size="56" fill="#EDE9FE">
    BIMI Quest
  </text>

  <!-- Tagline -->
  <text x="${WIDTH / 2}" y="340" text-anchor="middle"
        font-family="sans-serif" font-size="24" fill="#A78BFA">
    Certificate Market Intelligence
  </text>

  <!-- Description -->
  <text x="${WIDTH / 2}" y="400" text-anchor="middle"
        font-family="sans-serif" font-size="18" fill="#7C72B8">
    Track VMC and CMC certificate issuances across all Certificate Authorities
  </text>

  <!-- Bottom bar -->
  <rect x="0" y="${HEIGHT - 50}" width="${WIDTH}" height="50" fill="#15133A"/>
  <text x="40" y="${HEIGHT - 20}" font-family="sans-serif" font-size="16" fill="#4C1D95">
    bimi.quest
  </text>
</svg>`;
